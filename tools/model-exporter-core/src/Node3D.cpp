...
namespace
{
const float IDENTITY[] = { 1.0f, .0f, .0f, .0f, .0f, 1.0f, .0f, .0f, .0f, .0f, 1.0f, .0f, .0f, .0f, .0f, 1.0f };
}
...
Node3D::Node3D(Node3D *eParent)
{
    ...
    m_Index = -1;
    std::copy(std::begin(IDENTITY), std::end(IDENTITY), m_Matrix);
}
...
bool Node3D::IsMatrixIdentity() const
{
    for(int i = 0; i < 16; i++)
    {
        if( m_Matrix[i] == 1.0f)
