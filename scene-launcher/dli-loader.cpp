...
bool DliLoader::LoadAnimation( Actor toActor, std::vector<Animation> *animArray, std::string animation_name )
{
  ...
  animations = animations->GetChild( animation_name.c_str() );
...
bool DliLoader::LoadBuffer(const TreeNode* mesh, Geometry geometry, std::string& ebinFilename, unsigned char*& efileContent)
{
  ...
  if( !ReadInt( temp->GetChild( "byteOffset" ), byteOffset ) || !ReadInt( temp->GetChild( "byteLength" ), byteLength ) )
  {
    return false;
  }
