...
bool DliLoader::LoadBuffer(const TreeNode* mesh, Geometry geometry, std::string& ebinFilename, unsigned char*& efileContent)
{
  ...
  positionBuffer.SetData( &efileContent[byteOffset], byteLength / ( 3 * sizeof( float ) ) );
  ...
  normalBuffer.SetData( &efileContent[byteOffset], byteLength / ( 3 * sizeof( float ) ) );
  ...
  texCoordBuffer.SetData( &efileContent[byteOffset], byteLength / ( 2 * sizeof( float ) ) );
  ...
  tangentBuffer.SetData( &efileContent[byteOffset], byteLength / ( 3 * sizeof( float ) ) );
