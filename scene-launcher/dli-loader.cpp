...
const std::string DliLoader::LIGHTING_MODE_PROPERTY("lightingMode");
...
void DliLoader::AddNode( Actor toActor, const TreeNode *addnode, const std::vector<Shader>& shaderArray, LightingMode::Type lightMode )
{
  const TreeNode *itn = NULL;
  Actor actor;
  ...
  if ( auto node = addnode->GetChild(LIGHTING_MODE_PROPERTY) )  // overriding setting from parent?
  {
    if( node->GetType() == TreeNode::STRING )
    {
      if (!LightingMode::InterpretValue(node->GetString(), lightMode))
      {
        DALI_LOG_WARNING("Failed to interpret %s value '%s', keeping '%s'.\n", DliLoader::LIGHTING_MODE_PROPERTY.c_str(),
          value.c_str(), GetValue(lightMode));
      }
    }
    else if( node->GetType() == TreeNode::INTEGER )
    {
      ...
      {
        lightMode = static_cast<LightingMode::Type>(value);
      }
      else
      {
        DALI_LOG_WARNING("Ignoring invalid %s value (%d) in '%s'.\n", LIGHTING_MODE_PROPERTY.c_str(),
  ...
  actor.RegisterProperty(LIGHTING_MODE_PROPERTY, static_cast<int>(lightMode));
  ...
  if( actor )
  {
    ...
    if((itn = addnode->GetChild("children")))
    {
      ...
      for( unsigned int i = 0; i < children.Size(); ++i )
      {
        ...
        AddNode( actor, inodes, shaderArray, lightMode );
      }
    }
  }
}
