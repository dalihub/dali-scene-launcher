...
DliLoader::ShadowMode InterpretShadowMode(const std::string& value)
{
  ...
  {
    DALI_LOG_WARNING("Failed to interpret %s value '%s'.\n", DliLoader::SHADOW_MODE_PROPERTY.c_str(),
      value.c_str());
  }
  return result;
}
...
const std::string DliLoader::SHADOW_MODE_PROPERTY("shadowMode");
  ...
  if ( auto node = addnode->GetChild(SHADOW_MODE_PROPERTY) )  // setting from parent explicitly overridden?
  {
    ...
    else  //... but if present, it has to be valid type.
    {
      DALI_LOG_WARNING("Ignoring invalid %s type in '%s' (string or integer expected).\n",
        SHADOW_MODE_PROPERTY.c_str(), nodeName.c_str());
    }
  }
