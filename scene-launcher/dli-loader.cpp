...
void DliLoader::AddNode( Actor toActor, const TreeNode *addnode, const std::vector<Shader>& shaderArray, LightingMode::Type lightingMode )
{
  ...
  if ( auto node = addnode->GetChild(LightingMode::PROPERTY_NAME) )  // overriding setting from parent?
  {
    if( node->GetType() == TreeNode::STRING )
    {
      ...
      {
        DALI_LOG_WARNING("In node '%s': value '%s' is invalid for %s; keeping '%s'.\n", nodeName.c_str(),
            node->GetString().c_str(), LightingMode::PROPERTY_NAME, GetValue(lightingMode));
      }
    }
    else if( node->GetType() == TreeNode::INTEGER )
    {
      ...
      {
        DALI_LOG_WARNING("In node '%s': value %d is invalid for %s; keeping '%s'.\n", nodeName.c_str(),
            node->GetInteger(), LightingMode::PROPERTY_NAME, GetValue(lightingMode));
      }
    }
    else  //... but if present, it has to be valid type.
    {
      DALI_LOG_WARNING("In node '%s': %s has invalid type (string or integer expected).\n",
          nodeName.c_str(), LIGHTING_MODE_PROPERTY.c_str());
    }
  }
