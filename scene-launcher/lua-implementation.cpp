...
int ConnectKeyEvent( lua_State* state )
{
  ...
  void* sceneLauncherPtr = const_cast<void*>( lua_topointer( state, 1 ) );

  // Creates the connection with the OnKeyEvent method of the SceneLauncher.
  Stage stage = Stage::GetCurrent();
  stage.KeyEventSignal().Connect( reinterpret_cast<Scene3dLauncher*>( sceneLauncherPtr ), &Scene3dLauncher::OnKeyEvent );
...
int QuitApplication( lua_State* state )
{
  ...
  reinterpret_cast<Scene3dLauncher*>( sceneLauncherPtr )->ApplicationQuit();
