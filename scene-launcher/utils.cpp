...
Renderer CloneRenderer(Renderer r, uint32_t cloneOptions)
{
  Geometry g = r.GetGeometry();
  Shader s = MaskMatch(cloneOptions, CloneOptions::OMIT_SHADER) ? Shader() : r.GetShader();
...
Actor CloneActor(Actor a, uint32_t cloneOptions)
{
