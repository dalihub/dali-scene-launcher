...
#if defined ENABLE_RECEIVE_SHADOWS
// These shaders are for shadow render target debug.
/*
 * Vertex shader - flat
 */
...
void main()
{
  mediump vec4 color = vec4(texture2D( uTexture0, vUv0 ).r);
  gl_FragColor = color;
});
#endif
...
public:
  ...
  static Geometry NewUnitQuadWithWholeTexture()
  {
  ...
  static Actor CreateTexturedQuadActor(Texture texture, Shader shader)
  {
    TextureSet textures = TextureSet::New();
    textures.SetTexture(0, texture);

    // Create geometry and renderer.
    Geometry geom = NewUnitQuadWithWholeTexture();
...
#if defined ENABLE_RECEIVE_SHADOWS
    int shadowSize = 512;
    mShadowRenderTargetTexture = Texture::New(TextureType::TEXTURE_2D, Pixel::Format::A8, shadowSize, shadowSize);
    mShadowRenderTarget = FrameBuffer::New(shadowSize, shadowSize, 0);
...
//    mShadowCamera.SetTargetPosition(Vector3::ZERO);
    ...
    Actor shadowDebugActor = CreateTexturedQuadActor(mShadowRenderTargetTexture, plainOldShader);
#endif
  ...
  void InitShadowActor()
  {
    ...
    //mShadowModel.GetActor().RotateBy(Degree(180), Vector3(.0, .0, 1.0));

    // Replace shader with depth writing shader
  ...
  void InitActors()
  {
...
#if defined ENABLE_RECEIVE_SHADOWS
    mShadowCamera.SetOrthographicProjection(-1, 1, 1, -1, 1, -1);
    ...
    printf("ShadowCamera near: %f, far: %f, fov: %f\n", mShadowCamera.GetNearClippingPlane(), mShadowCamera.GetFarClippingPlane(), mShadowCamera.GetFieldOfView());
...
#if defined ENABLE_RECEIVE_SHADOWS
    Actor light = mModel.GetActor().FindChildByName("Light");
    if (light)
    {
      mShadowCamera.SetPosition(light.GetProperty(Actor::Property::POSITION).Get<Vector3>());
      mShadowCamera.SetOrientation(light.GetProperty(Actor::Property::ORIENTATION).Get<Quaternion>());
    }
    ...
    // <REMOVEME> This is to enable rendering the original actors with the shadow shader, to the main framebuffer.
...
private:
  ...
  {
    Matrix mProjection = cam.GetProperty(CameraActor::Property::PROJECTION_MATRIX).Get<Matrix>();
    Matrix mLightSpaceTransform;
    Matrix::Multiply(mLightSpaceTransform, cam.GetCurrentWorldMatrix(), mProjection);
    //mLightSpaceTransform.Invert();

    float const* data = mLightSpaceTransform.AsFloat();
    ...
    {
      t.actor.SetProperty(t.uLightSpaceTransform, mLightSpaceTransform);
    }
  }
#endif
