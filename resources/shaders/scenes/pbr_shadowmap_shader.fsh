...
const float Bias = pow(2.0, -8.0);

float CalculateShadow(vec4 lightSpacePos, vec3 normal)
{
  // Perform manual perspective divide since we didn't get this via gl_Position
  vec3 lightSpaceProjectedPos = lightSpacePos.xyz / lightSpacePos.w;

  // Transform to NDC.
  lightSpaceProjectedPos = lightSpaceProjectedPos * .5 + .5;

  // Sample depth.
  float shadowClosest = texture(sShadow, lightSpaceProjectedPos.xy).r;
  float bias = max(1.0 + dot(normal, uLightDir), 1.0);
  bias *= Bias;
  float shadowComponent = (lightSpaceProjectedPos.z - bias < shadowClosest) ? 1.0 : 0.0;
