...
const vec3 PoissonDiscWeights[] = vec3[13](  // x, y - offset in texels; z - weight
  vec3(.0, .0, 8.),
  vec3(-.5, -.5, 4.),
  vec3(-.5, .5, 4.),
  vec3(.5, -.5, 4.),
...
float CalculateShadow(vec3 lightSpaceProjectedPos, vec3 normal)
{
  float shadowAccum = .0;
  float bias = dot(normal, uLightDir);
  float totalWeight = .0;
  bias *= Bias;
  for(int i = 0; i < 13; ++i)
  {
    vec2 moments = texture(sShadow, lightSpaceProjectedPos.xy + PoissonDiscWeights[i].xy * uInverseTextureSize).rg;
    float weight = PoissonDiscWeights[i].z;
    float distance = lightSpaceProjectedPos.z - bias;
    if (distance <= moments.x)  // Fragment is in front of any occluders
    {
      shadowAccum += weight;
    }
    else  // Use Chebyshev probability calculation to see if we might be occluded.
    {
      ...
      probability *= probability; // Darker shadows / penumbra
      shadowAccum += probability * weight;
    }
    totalWeight += weight;
  }
...
//finalLod = 4.0;
  ...
  float shadowComponent = CalculateShadow(vLightSpacePos, vNormal);
  
  // For increasing contrast a little bit - It should be Gamma correction (2.2) but it gets to dark
  albedoColor = pow( albedoColor, vec3( Gamma ) ) * shadowComponent;
...
//  FragColor = vec4( finalColor * .0001 + vec3(shadowComponent), 1.0 );
